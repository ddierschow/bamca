...
function show_div($css, $scr, $img, $lbl, $ttl='') {
    ...
    echo '   <div class="tabm">' . "\n";
    if ($img && file_exists('.' . $IMG_DIR_ICON . '/t_' . $img)) {
	echo '    <img src="' . $IMG_DIR_ICON . '/t_' . $img . '" title="' . $lbl . '">' . "\n";
    } else {
	echo '    ' . $lbl . "\n";
    }
    if ($ttl) {
...
show_div('bk',  $mans . 'bk',     'battlekings.gif', 'Battle Kings');
show_div('a2k', $mans . 'a2k',    'adventure2k.gif', 'Adventure 2000');
...
show_div('dy',  $mans . 'dy',     'dinky.gif', 'Dinky Collection');
...
show_div('acc', $mans . 'acc',    'access.gif', 'Accessories');
