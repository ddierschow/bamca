...
def print_user_form(pif, id):
    ...
    lsection = dict(columns=cols, headers=heads, range=[lrange],
		    header='<form name="userform" method="post" action="/cgi-bin/user.cgi">' + pif.create_token(), note='')
    llineup = dict(section=[lsection],
	footer=pif.render.format_button_input("save changes", "submit") + ' -\n' +
	       pif.render.format_button_reset("userform") + ' -\n' +
	       pif.render.format_button("change password", pif.secure_host + "/cgi-bin/chpass.cgi?id=%s" % id) + '</form>')
    return pif.render.format_template('simplelistix.html', llineup=llineup)
...
<a href="{secure_host}/cgi-bin/validate.cgi?user_id={user_id}&vkey={vkey}">{secure_host}/cgi-bin/validate.cgi?user_id={user_id}&vkey={vkey}</a>
...
<a href="{secure_host}/cgi-bin/recover.cgi?user_id={user_id}&vkey={vkey}">{secure_host}/cgi-bin/recover.cgi?user_id={user_id}&vkey={vkey}</a>
