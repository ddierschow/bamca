...
def check_matrix_model(pif):
    ...
    res = pif.dbh.raw_execute(
        '''select page_id, section_id, range_id, count(*) from matrix_model group by page_id, section_id, range_id''')
    check_q(res[0])
...
def check_base_id(pif):
    ...
    for id in ids:
        ...
        if (id['base_id.first_year'] is None or
                int(id['base_id.first_year']) < 1947 or int(id['base_id.first_year']) > 2015):
...
def check_var_vs_categories(pif):
    ...
    vl = [key + (' '.join(sorted(vars[key]['vc'])), ' '.join(sorted(vars[key]['vsc'])),)
...
def cat2matrix(pif, cat, page_id):
    ...
    for vvs in pif.dbh.fetch('variation_select vs,variation v,base_id b', where=wheres, tag='VarVSCat'):
        ...
        if iam not in seen:
            print(fmt % ("", cat.lower() + vvs['v.date'][2:4], ran_id, page_id, ran_id, vvs['v.mod_id'], 0, "",
                         vvs['b.rawname'], '', vvs['v.text_description']))
            ran_id += 1
            seen.add(iam)
...
def check_vs(pif):
    print("check vs")
    res = pif.dbh.raw_execute(
        '''select vs.*, v.* from variation_select vs left join variation v on vs.mod_id=v.mod_id and vs.var_id=v.var''')
    for ent in res[0]:
        if not ent[6]:
            print(ent[:6])
    print()
    ...
    for ref in refs:
        ...
        for varsel in varsels:
            ...
            if not var:
                continue
            if pif.switch['l']:
                print(var['variation.mod_id'], var['variation.var'], var['variation.category'], '/',
...
def check_photogs(pif):
    for phot in pif.dbh.fetch_photographers():
        if not phot['example_id']:
            ...
            if creds:
                print(ph_id, pif.dbh.write('photographer', values={'example_id': creds[0]['photo_credit.id']},
                                           where='id="%s"' % ph_id, verbose=True, modonly=True))
            else:
...
def check_images(pif):
    ...
    variation_ids = [x['variation.mod_id'].lower() + '-' + x['variation.var'].lower()
    ...
    for lm in pif.dbh.fetch_simple_lineup_models(region=mbdata.regionlist):
        if lm['lineup_model.picture_id']:
            ...
            if num in files.get(region, {}).get(year, ['00']):
                print('hidden image', year, lm['lineup_model.region'], lm['lineup_model.number'],
                      lm['lineup_model.picture_id'])
            else:
