...
def render_cars(pif, cf):
    ...
    for c in cf.sec:
        ...
        print(pif.ren.format_cell(sec, c[0], hdr=True, also={'colspan': len(c[1][0])}))
        sec = sec + len(c[1][0])
    ...
    for i in range(0, imax):
        ...
        for c in cf.sec:
            if i >= len(c[1]):
                for f in c[1][0]:
                    print(pif.ren.format_cell(sec, ' '))
                    sec = sec + 1
            else:
                for f in c[1][i]:
                    if f == 'x':
                        print(pif.ren.format_cell(sec, pif.ren.format_image_art('box-sm-x.gif')))
                    elif f:
                        print(pif.ren.format_cell(sec, "&nbsp;" + f + "&nbsp;"))
                    else:
                        print(pif.ren.format_cell(sec, pif.ren.format_image_art('box-sm.gif')))
                    sec = sec + 1
        print(pif.ren.format_row_end())
